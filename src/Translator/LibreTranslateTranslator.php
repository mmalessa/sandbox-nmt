...
class LibreTranslateTranslator implements Translator
{
    public function __construct(
        private readonly string $endpoint = 'http://libretranslate:5000/translate',
        ...
        for ($attempt = 1; $attempt <= $this->maxRetries; $attempt++) {
            try {
                ...
                if ($attempt === $this->maxRetries) {
                    echo "LT TRANSLATE {$source}->{$target} ERROR (after {$this->maxRetries} attempts) " . $e->getMessage() . PHP_EOL;
                    return '';
                }
