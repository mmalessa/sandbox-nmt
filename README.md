# Sandbox LibreTranslate

A PHP sandbox for experimenting with [LibreTranslate](https://libretranslate.com/) — a self-hosted machine translation engine. 
...
- **libretranslate** — LibreTranslate server (port 5000) with a persistent volume for language models
- **php** — PHP 8.5 CLI (Alpine) with Composer 2, mounting the project directory as `/app`
...
The script translates a given phrase from the source language into all configured target languages and returns the result as JSON.
...
make sh      # Enter the PHP container shell
...
- PHP 8.5 (extensions: zip, pcntl, intl, bcmath)
...
- XDebug pre-configured for PHPStorm (`host.docker.internal:9003`)
